...
all_the_dump_files = [f for f in os.listdir() if f.endswith('.dump') and not os.path.isdir(f)]
...
for data_file in all_the_dump_files:
    ...
    for row, line in enumerate(lines):
        ...
        if check_for_mod in line:
            print(f'{data_file} has already been modified.\n')
            mod_flag = True
            break
    ...
    measure_name = f'{file_stem}_measurements.txt'
    ...
    for csv_file in glob.glob('CNT_*.csv'):
        ...
        distance_xu = math.sqrt((max_xu - min_xu) ** 2)      ## Use for Diameter x direction
        distance_yu = math.sqrt((max_yu - min_yu) ** 2)      ## Use for Diameter y direction
        distance_zu = math.sqrt((max_zu - min_zu) ** 2)      ## Use for Length
        ## Convert float to string to write
    ...
    if mod_flag:
        ...
        ## Initalize CNT directory name, will give a FileExistsError if directory already exists at the destination
